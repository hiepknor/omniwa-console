...
const resources = Object.fromEntries(await Promise.all(Object.entries({
  app: new URL('../src/app/App.tsx', import.meta.url), page: new URL('../src/features/groups-v2/GroupsPageV2.tsx', import.meta.url),
...
for (const required of ["searchParams.get('cursor')", "searchParams.get('search')"]) if (!resources.route.includes(required)) failures.push(`Groups route state is missing ${required}`);
