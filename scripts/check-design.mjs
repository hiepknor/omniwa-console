...
for (const marker of ['autoGrow', 'maxRows={4}', 'grid-cols-[minmax(0,1fr)_auto]', 'aria-label={`Message ${conversationName}`}', 'placeholder="Write a message…"', 'aria-label="Choose image or media"', '>Media…</Button>', ": 'Send'}"]) {
...
for (const marker of ['grid-cols-[320px_minmax(0,1fr)]', 'max-[900px]:grid-cols-1', "detailOpen && 'max-[900px]:hidden'", "!detailOpen && 'max-[900px]:hidden'", "detailInitialPosition = 'start'", 'detailScrollerRef', "position === 'end' ? ref.current.scrollHeight : 0", "frame === 'standalone'", 'WorkspacePaneHeader']) {
...
for (const marker of ['<WorkspacePageFrame', 'title="Directory"', '<Tabs', 'panelId: \'directory-preview\'', '<SplitWorkspace', 'detailOpen={Boolean(selectedId)}', 'icon="arrow-left"', '<CountBadge count={filtered.length}', '<ContactList', '<LabelList', '<DirectoryDetails']) {
...
for (const marker of ['<WorkspacePageFrame', 'title="Directory"', '<Tabs', 'panelId: \'directory-panel\'', 'role="tabpanel"', '<SplitWorkspace', 'detailOpen={Boolean(selectedId)}', 'icon="arrow-left"', '<CountBadge count={total}', 'Search contacts', 'Filter labels', '<ContactList', '<LabelList', '<DirectoryDetails', "cap('contacts_projection')", "cap('labels_projection')", "cap('canonical_contact_identity')"]) {
