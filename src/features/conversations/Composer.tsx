...
export function Composer({ chatId, recipient, chatName, enabled, mediaEnabled, unavailableDetail }: { chatId: string; recipient: string; chatName: string; enabled: boolean; mediaEnabled: boolean; unavailableDetail?: string }) {
  ...
  const sendSelectedMedia = () => {
    if (!enabled || sendMedia.isPending) return;
    sendMedia.reset();
    ...
    <div className="grid gap-3 border-t border-line bg-surface p-4">
      ...
      <form className="grid gap-2" onSubmit={(event) => { event.preventDefault(); submitText(); }}>
        <Field label={`Message ${chatName}`}>
          {(id) => <Textarea id={id} rows={3} value={text} disabled={!enabled || pending} maxLength={10_000} onChange={(event) => { setText(event.target.value); if (sendText.error) sendText.reset(); }} />}
        </Field>
        <div className="flex justify-end gap-2">
          <Button disabled={!enabled || pending} onClick={() => { resetMedia(); setMediaOpen(true); }}>Image or media…</Button>
          <Button variant="primary" type="submit" disabled={!enabled || !text.trim() || pending || textOutcomeUnknown}>{sendText.isPending ? 'Submitting…' : 'Send text'}</Button>
        </div>
      </form>
      ...
      <Dialog
        ...
        footer={sendMedia.data
          ...
          : <><Button disabled={pending} onClick={closeMedia}>Cancel</Button><Button variant="primary" disabled={!canSendMedia || pending || mediaOutcomeUnknown} onClick={sendSelectedMedia}>{sendMedia.isPending ? 'Submitting…' : 'Send media'}</Button></>}
      >
        <div className="grid gap-4">
          <Tabs active={source} onChange={(id) => { setSource(id as 'device' | 'url'); sendMedia.reset(); }} tabs={[{ id: 'device', label: 'Device image' }, { id: 'url', label: 'Remote URL' }]} />
