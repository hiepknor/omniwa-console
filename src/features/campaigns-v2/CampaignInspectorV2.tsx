  ...
  return <>
    <Inspector titleId="campaign-v2-title" eyebrow="Campaign" title={campaign?.name ?? 'Campaign detail'} subtitle={<span className="ui-v2-mono">{campaignId}</span>} status={campaign ? <Status tone={tone(campaign.status)}>{humanizeToken(campaign.status)}</Status> : undefined} modal onClose={onClose}>
      {detail.isPending ? <StateNotice value={{ axis: 'resource', state: 'initial-loading' }} /> : detail.error || !detail.data || !campaign ? <ApiFailureNotice error={detail.error ?? new Error('Campaign detail unavailable.')} onRetry={() => detail.refetch()} /> : <div className="ui-v2-stack">
        ...
        {route.tab === 'recipients' ? <div id="recipients-panel" role="tabpanel" aria-labelledby="recipients-tab"><PagedSection pending={recipients.isPending} error={recipients.error} retry={() => recipients.refetch()}>{recipients.data ? <><Table ariaLabel="Campaign recipients" caption="Campaign recipients" className="ui-v2-campaign-recipient-table" rows={recipients.data.items} rowKey={(item) => item.id} columns={[{ header: 'Recipient', cell: (item) => <><span className="ui-v2-mono">{item.jid}</span><small>{item.optInSource || 'Source unreported'}</small></> }, { header: 'Status', cell: (item) => <Status tone={tone(item.status)}>{humanizeToken(item.status)}</Status> }, { header: 'Attempts', cell: (item) => item.attemptCount }, { header: 'Updated', cell: (item) => relativeTime(item.updatedAt) || 'Not reported' }]} />{!recipients.data.items.length ? <StateNotice value={{ axis: 'resource', state: 'empty' }} detail="No campaign recipients were returned." /> : null}<CursorPagination cursor={route.recipientCursor} nextCursor={recipients.data.nextCursor} onCursor={(value) => setParam('recipientCursor', value)} /></> : null}</PagedSection></div> : null}
        {route.tab === 'audit' ? <div id="audit-panel" role="tabpanel" aria-labelledby="audit-tab"><PagedSection pending={audit.isPending} error={audit.error} retry={() => audit.refetch()}>{audit.data ? <><ol className="ui-v2-campaign-audit">{audit.data.items.map((item) => <li key={item.id}><Status tone="neutral">{humanizeToken(item.eventType || 'event')}</Status><strong>{item.fromStatus && item.toStatus ? `${humanizeToken(item.fromStatus)} → ${humanizeToken(item.toStatus)}` : humanizeToken(item.actorType || 'system')}</strong><span>{relativeTime(item.occurredAt) || 'Time unreported'}</span></li>)}</ol>{!audit.data.items.length ? <StateNotice value={{ axis: 'resource', state: 'empty' }} detail="No campaign audit entries were returned." /> : null}<CursorPagination cursor={route.auditCursor} nextCursor={audit.data.nextCursor} onCursor={(value) => setParam('auditCursor', value)} /></> : null}</PagedSection></div> : null}
      </div>}
    </Inspector>
