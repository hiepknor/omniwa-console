...
import { Button, CursorPagination, Field, PageGuard, PageHeader, StateNotice, Status, Surface, Tabs } from '@/components/v2';
...
export function ConversationsPageV2() {
  ...
  const switchView = (view: ConversationViewV2) => {
    navigate(withSearchParams('/chats', createSearchParams({ view: view === 'chats' ? undefined : view })));
  };
  const openChat = (id: string) => {
    const next = omitSearchParams(searchParams, ['message', 'messageCursor', 'selected']);
    navigate(withSearchParams(`/chats/${encodeURIComponent(id)}`, next));
  };
    ...
    <div className="ui-v2-page__content">
      <div className="ui-v2-conversation-workspace" data-has-chat={Boolean(activeChatId) || undefined}>
        <Surface title="Directory" description="Selection, search, and opaque cursor remain URL-backed." className="ui-v2-conversation-directory">
          ...
          {route.view !== 'labels' && viewSupported && currentQuery.data ? <CursorPagination cursor={route.cursor} nextCursor={route.view === 'chats' ? chats.data?.resource.pagination.nextCursor : contacts.data?.resource.pagination.nextCursor} onCursor={(value) => replaceParams(updateSearchParams(searchParams, { cursor: value }, ['selected']))} /> : null}
        </Surface>
