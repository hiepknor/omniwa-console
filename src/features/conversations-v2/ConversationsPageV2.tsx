      ...
      <div className="flex-1 min-h-0 grid grid-cols-[320px_minmax(0,1fr)] border-t border-line max-[900px]:grid-cols-1">
        ...
        <section className={cn('flex flex-col min-h-0 border-r border-line', hasChat && 'max-[900px]:hidden')}>
          <div className="shrink-0 border-b border-line">
            <Tabs
              active={route.view}
              onChange={(id) => switchView(id as ConversationViewV2)}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            ...
            {viewSupported && currentQuery.data ? (
              <>
                {currentQuery.error ? <div className="p-3"><FailureNoticeV2 error={currentQuery.error} stale onRetry={refresh} /></div> : null}
                <div className="px-3"><ProjectionStatusV2 meta={'meta' in currentQuery.data ? currentQuery.data.meta : undefined} /></div>
        ...
        <section className={cn('flex flex-col min-h-0', !hasChat && 'max-[900px]:hidden')}>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            {!activeChatId ? (
              ...
              <div className="p-4"><FailureNoticeV2 error={chat.error} onRetry={() => chat.refetch()} /></div>
            ) : selectedChat ? (
              <>
                <div className="px-4"><ProjectionStatusV2 meta={chat.data?.meta} /></div>
                ...
                {!messagesSupported ? (
                  ...
                  <div className="p-4"><FailureNoticeV2 error={messages.error} onRetry={() => messages.refetch()} /></div>
                ) : messages.data ? (
                  <>
          ...
          {selectedChat ? <div className="shrink-0"><ComposerV2 chatId={selectedChat.id} chatName={selectedChat.displayName ?? selectedChat.id} enabled={messagesReady && outboundReady} /></div> : null}
        </section>
      </div>
