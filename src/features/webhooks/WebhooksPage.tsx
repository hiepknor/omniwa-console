...
export function WebhooksPage() {
  ...
  const tableState: DataTableState<WebhookRow> = listReadState.isInitialError ? { status: 'error', error: listReadState.error, onRetry: list.refetch } : listReadState.isInitialLoading ? { status: 'loading', skeletonRows: 6 } : unavailable && webhooks.length === 0 ? { status: 'unavailable', message: 'Webhook data is not available yet.' } : filtered.length === 0 ? { status: 'empty', message: webhooks.length ? 'No webhooks match these filters.' : 'No webhooks registered.' } : { status: 'ready', rows: filtered };
