...
import { WebhookDrawer, WebhookDrawerState, webhookStatusDot } from './WebhookDrawer';
...
export function WebhooksPage() {
  ...
  const list = useWebhooks(initialCursor);
  ...
  const previousWebhookId = useRef(webhookId);

  useEffect(() => {
    if (previousWebhookId.current === webhookId) return;
    previousWebhookId.current = webhookId;
    const next = new URLSearchParams(searchParams);
    next.delete('delivery');
    setSearchParams(next, { replace: true });
  }, [searchParams, setSearchParams, webhookId]);

  const setParam = (name: string, value: string) => { const next = new URLSearchParams(searchParams); if (value) next.set(name, value); else next.delete(name); if (name === 'search' || name === 'status') next.delete('cursor'); setSearchParams(next, { replace: true }); };
  const querySuffix = searchParams.size ? `?${searchParams.toString()}` : '';
  const closeDrawer = () => { const next = new URLSearchParams(searchParams); next.delete('delivery'); navigate(`/webhooks${next.size ? `?${next.toString()}` : ''}`); };
  ...
  const tableState: DataTableState<WebhookRow> = listReadState.isInitialError ? { status: 'error', error: listReadState.error, onRetry: list.refetch } : listReadState.isInitialLoading ? { status: 'loading', skeletonRows: 6 } : unavailable && webhooks.length === 0 ? { status: 'unavailable', message: 'Webhooks are not available on OmniWA GO.' } : filtered.length === 0 ? { status: 'empty', message: webhooks.length ? 'No webhooks match these filters.' : 'No webhooks registered.' } : { status: 'ready', rows: filtered };
  ...
  return <>
    ...
    {webhookId && (detail.data?.resource ? <WebhookDrawer key={detail.data.resource.id} webhook={detail.data.resource} onClose={closeDrawer} onRetired={(result) => { feedback.command(result.disposition, { action: 'Retire webhook', acceptedDetail: 'The platform accepted the command. The list refreshes automatically.', completedDetail: 'The platform retired the webhook. The list refreshes automatically.', requestId: result.requestId, dedupeKey: `webhook:${webhookId}:retire` }); closeDrawer(); }} /> : detail.data?.unavailable ? <WebhookDrawerState webhookId={webhookId} onClose={closeDrawer}>Webhooks are not available on OmniWA GO.</WebhookDrawerState> : detail.isError ? <WebhookDrawerState webhookId={webhookId} onClose={closeDrawer}><InlineError error={detail.error} onRetry={detail.refetch} /></WebhookDrawerState> : <WebhookDrawerState webhookId={webhookId} onClose={closeDrawer} announce>Loading webhook details…</WebhookDrawerState>)}
    {registerOpen && <RegisterWebhookDialog error={register.error} isPending={register.isPending} onCancel={() => setRegisterOpen(false)} onRegister={(body) => register.mutate(body, { onSuccess: (result) => { setRegisterOpen(false); feedback.command(result.disposition, { action: 'Register webhook', acceptedDetail: 'The platform accepted the command. The webhook will appear after it is recorded.', completedDetail: 'The platform registered the webhook.', requestId: result.requestId, dedupeKey: 'webhook:register' }); } })} />}
  </>;
}
