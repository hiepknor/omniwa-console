...
export function WebhookDrawer({ webhook, onClose, onRetired }: { webhook: WebhookResource; onClose: () => void; onRetired: (result: CommandResult) => void }) {
  ...
  return <>
    <DetailDrawer titleId="webhook-detail-title" eyebrow="Webhook management" title="Webhook endpoint" status={<span className="status"><span className={`dot ${webhookStatusDot(webhook.status)}`} />{webhook.status ?? '—'}</span>} subtitle={<DrawerIdentifier value={webhook.id} label="Copy webhook identifier" />} className="webhook-drawer" closeLabel="Close webhook details" suppressEscape={retireOpen} onClose={onClose}>
        <section aria-labelledby="webhook-facts-title"><h3 id="webhook-facts-title" className="visually-hidden">Webhook facts</h3><dl className="kv"><dt>Events</dt><dd><div className="capchips">{(webhook.eventTypes ?? []).length ? webhook.eventTypes?.map((event) => <span className="chip" key={event}>{event}</span>) : '—'}</div></dd><dt>Created</dt><dd className="ts" title={webhook.createdAt}>{relativeTime(webhook.createdAt) || '—'}</dd><dt>Updated</dt><dd className="ts" title={webhook.updatedAt}>{relativeTime(webhook.updatedAt) || '—'}</dd></dl></section>
