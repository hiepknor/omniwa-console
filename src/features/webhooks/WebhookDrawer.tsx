...
function DeliveryRow({ delivery, selected, checked, busy, onSelect, onCheck, onRetry, onRedrive }: {
  ...
  return <article className={`delivery-record${selected ? ' selected' : ''}`}>
    <dl>
      ...
      <dt>Event</dt><dd><DrawerTechnicalValue value={delivery.eventType} /></dd>
      <dt>Status</dt><dd><StatusIndicator size="small" dotClass={webhookStatusDot(delivery.status)}>{delivery.status ?? '—'}</StatusIndicator></dd>
  ...
  return <>
    <DetailDrawer titleId="webhook-detail-title" eyebrow="Webhook management" title="Webhook endpoint" status={<StatusIndicator dotClass={webhookStatusDot(webhook.status)}>{webhook.status ?? '—'}</StatusIndicator>} subtitle={<DrawerIdentifier value={webhook.id} label="Copy webhook identifier" />} className="webhook-drawer" closeLabel="Close webhook details" suppressEscape={retireOpen} onClose={onClose}>
        ...
        <section aria-labelledby="webhook-deliveries-title"><div className="drawer-section-head"><div><span className="eyebrow">Loaded records</span><h3 id="webhook-deliveries-title">Recent deliveries</h3></div><span className="num delivery-count">{deliveries.length}</span></div>
          ...
          {(retry.error ?? redrive.error) && <InlineError error={retry.error ?? redrive.error} announce onRetry={() => { if (retry.error && retry.variables) retry.mutate(retry.variables); else if (redrive.variables) redrive.mutate(redrive.variables); }} />}
        </section>
        <section className="webhook-recovery" aria-labelledby="webhook-recovery-title"><span className="eyebrow">Bulk recovery</span><h3 id="webhook-recovery-title">Failed deliveries</h3><div className="recovery-action"><span>{checked.size ? `${checked.size} selected` : `${failed.length} failed in loaded data`}</span><button className="btn" type="button" disabled={!checked.size || pending} onClick={() => bulk.mutate([...checked], { onSuccess: (result) => { commandFeedback(result, 'Bulk redrive', 'bulk-redrive'); setChecked(new Set()); } })}>{bulk.isPending ? 'Submitting…' : 'Redrive selected'}</button></div>{bulk.error && <InlineError error={bulk.error} announce onRetry={() => { if (bulk.variables) bulk.mutate(bulk.variables); }} />}</section>
        {webhook.status !== 'retired' && <section aria-labelledby="webhook-danger-title"><span className="eyebrow">Danger zone</span><h3 id="webhook-danger-title">Retire endpoint</h3><div className="action-group destructive"><span>Permanently stop future deliveries after typed confirmation.</span><button className="btn danger" type="button" onClick={() => setRetireOpen(true)}>Retire…</button></div></section>}
    </DetailDrawer>
...
export function WebhookDrawerState({ webhookId, onClose, children, announce = false }: { webhookId: string; onClose: () => void; children: React.ReactNode; announce?: boolean }) {
  return <DetailDrawer titleId="webhook-detail-title" eyebrow="Webhook management" title="Webhook details" subtitle={<DrawerIdentifier value={webhookId} label="Copy webhook identifier" />} className="webhook-drawer" closeLabel="Close webhook details" onClose={onClose}><DetailDrawerState announce={announce}>{children}</DetailDrawerState></DetailDrawer>;
}
