...
import { Button, ButtonLink, CursorPagination, DescriptionItem, DescriptionList, Dialog, Drawer, Field, Image, Input, Panel, Select, StateNotice, Status, Switch, Tabs, Textarea, type Tone } from '@/ui';
import {
...
function GroupWorkspaceContent({ group, normalized, commandsEnabled, membersEnabled, auditEnabled, photoEnabled, activeTab, memberSearch, memberRole, memberCursor, auditCursor, onParam, onTab, onLeft }: {
  ...
  const invite = useGroupInvite(group.id, activeTab === 'settings' && canReadInvite);
...
export function GroupOverview({ group, normalized }: { group: GroupResource; normalized: boolean }) {
  ...
  {normalized ? <Panel title="Action preflight" description="Advisory decisions may change; every command is revalidated by the backend."><div className="grid grid-cols-2 gap-x-4 max-sm:grid-cols-1">{actionLabels.map(([name, label]) => { const decision = group.actions?.[name]; return <div key={name} className="flex items-center justify-between gap-3 border-b border-line py-2"><span className="text-sm">{label}</span><Status tone={decision?.state === 'allowed' ? 'ok' : decision?.state === 'denied' ? 'failed' : 'degraded'}>{decisionText(decision)}</Status></div>; })}</div></Panel> : null}
...
function SettingsPanels({ group, normalized, commandsEnabled, photoEnabled, subject, description, invite, updateName, updateDescription, setting, onSubject, onDescription, onCommand, onResetInvite, onLeave }: { group: GroupResource; normalized: boolean; commandsEnabled: boolean; photoEnabled: boolean; subject: string; description: string; invite: ReturnType<typeof useGroupInvite>; updateName: MutationLike<string>; updateDescription: MutationLike<string>; setting: ReturnType<typeof useUpdateGroupSetting>; onSubject: (v: string) => void; onDescription: (v: string) => void; onCommand: (result: GroupCommandAcknowledgement) => void; onResetInvite: () => void; onLeave: () => void }) {
  ...
  <Panel title="Invite link" description="Read and reset use separate advisory decisions."><div className="grid gap-3">{!allowed(group.actions?.readInviteLink, normalized) ? <StateNotice kind="empty" title="Invite unavailable" detail={decisionText(group.actions?.readInviteLink)} /> : invite.isPending ? <StateNotice kind="loading" title="Loading invite" /> : invite.error && !invite.data ? <Fail error={invite.error} onRetry={() => invite.refetch()} /> : <code className="block break-all border border-line bg-recessed p-2 font-mono text-xs">{invite.data ?? 'No invite link reported'}</code>}<Button variant="danger" className="justify-self-start max-sm:w-full" disabled={!canResetInvite} title={!canResetInvite ? decisionText(group.actions?.resetInviteLink) : undefined} onClick={onResetInvite}>Reset invite link…</Button></div></Panel>
