...
describe('Group List route state', () => {
  it('keeps list and nested cursors opaque and normalizes tabs', () => {
    expect(groupListRouteState(new URLSearchParams('search=North&cursor=a%2Fb&groupCursor=g%3A1&auditCursor=x%2By&tab=audit&notice=updated'))).toEqual(expect.objectContaining({ search: 'North', cursor: 'a/b', groupCursor: 'g:1', auditCursor: 'x+y', tab: 'audit', notice: 'updated' }));
  });
