...
export function GroupListsPage() {
  ...
  return (
    <>
      ...
      {groupListId ? <GroupListInspector id={groupListId} enabled={enabled} eligibilityEnabled={eligibilityEnabled} route={route} params={params} setParam={setParam} onClose={() => navigate(`/groups/lists${route.search ? `?search=${encodeURIComponent(route.search)}` : ''}`)} /> : null}
    </>
  );
}

function GroupListInspector({ id, enabled, eligibilityEnabled, route, setParam, onClose }: { id: string; enabled: boolean; eligibilityEnabled: boolean; route: ReturnType<typeof groupListRouteState>; params: URLSearchParams; setParam: (key: string, value?: string) => void; onClose: () => void }) {
  const navigate = useNavigate();
