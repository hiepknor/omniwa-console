...
export function DirectoryDetails({ contact, label, meta, error, loading, onRetry }: { contact?: ContactResource; label?: LabelResource; meta?: ProjectionMeta; error?: unknown; loading: boolean; onRetry: () => void }) {
  ...
  if (label) return (
    <div className="grid gap-4 p-4">
      {error ? <FailureNotice error={error} stale onRetry={onRetry} /> : null}
      <ProjectionStatus meta={meta} />
      <Panel headingLevel={2} title="Projected definition" description="Definitions are read-only; Console does not infer Conversation or Message assignments." bodyPadding="compact-top">
        <DescriptionList>
  ...
  return <div className="p-4"><StateNotice kind="empty" title="Not returned" detail="The selected projected resource was not returned." /></div>;
}
