...
export function FilterChip({ label, value, onRemove }: { label: string; value: ReactNode; onRemove: () => void }) {
  return (
    <button type="button" aria-label={`Remove ${label} filter`} onClick={onRemove} className="group inline-flex h-8 max-w-full items-center border border-line-strong bg-surface text-xs text-fg transition-colors hover:bg-elevated focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">
      ...
      <svg aria-hidden viewBox="0 0 16 16" className="mr-2 size-3 shrink-0 text-fg-3 group-hover:text-fg" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="square">
