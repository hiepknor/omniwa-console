...
export function Radio({ label, description, className, disabled, id: providedId, ...props }: ChoiceProps) {
  const generatedId = useId();
  const id = providedId ?? generatedId;
  const labelId = `${id}-label`;
  const descriptionId = description ? `${id}-description` : undefined;
  return (
    <label htmlFor={id} className={cn('flex min-h-9 cursor-pointer items-start gap-2 py-2 max-sm:min-h-10', disabled && 'cursor-not-allowed opacity-60', className)}>
      <span className="relative mt-0.5 size-4 shrink-0">
        <input id={id} type="radio" aria-labelledby={labelId} aria-describedby={descriptionId} disabled={disabled} className="peer absolute inset-0 size-full cursor-inherit appearance-none opacity-0" {...props} />
        <span aria-hidden className="pointer-events-none grid size-4 place-items-center border border-line-strong bg-surface after:size-1.5 after:bg-fg after:opacity-0 after:content-[''] peer-checked:after:opacity-100 peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-accent peer-disabled:border-line peer-disabled:bg-elevated" />
      </span>
      <span className="grid min-w-0 gap-0.5">
        <strong id={labelId} className="text-[13px] font-medium leading-4 text-fg">{label}</strong>
        {description ? <small id={descriptionId} className="text-xs leading-4 text-fg-3">{description}</small> : null}
